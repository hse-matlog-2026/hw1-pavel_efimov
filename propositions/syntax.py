...
class Formula:
    ...
    def _parse_prefix(string: str) -> Tuple[Union[Formula, None], str]:
        ...
        f1, r1 = Formula._parse_prefix(s[1:])
        if f1 is None:
            return None, r1

        op = None
        lim = min(3, len(r1))
        for ln in range(lim, 0, -1):
            cur = r1[:ln]
            if is_binary(cur):
                op = cur
                break
        ...
        f2, r2 = Formula._parse_prefix(r1[len(op):])
        if f2 is None:
            return None, r2
        if len(r2) == 0 or r2[0] != ')':
            return None, "missing ')'"

        return Formula(op, f1, f2), r2[1:]
    ...
    def parse_polish(string: str) -> Formula:
        ...
        """
        # Optional Task 1.8
        def parse_from(s: str) -> Tuple[Union[Formula, None], str]:
            if len(s) == 0:
                return None, 'empty'
            ...
            if s[0] == '~':
                f1, r1 = parse_from(s[1:])
                if f1 is None:
                    return None, r1
                return Formula('~', f1), r1
            ...
            for ln in range(lim, 0, -1):
                ...
                if is_binary(cur):
                    ...
                    break
            if op is None:
                return None, 'bad operator'

            f1, r1 = parse_from(s[len(op):])
            if f1 is None:
                return None, r1
            f2, r2 = parse_from(r1)
            if f2 is None:
